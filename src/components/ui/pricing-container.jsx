...
const PricingCard = ({
    ...
    return (
        <motion.div
            ...
            {/* Price Badge */}
            <motion.div
                className={cn(`absolute -top-4 -right-4 w-16 h-16 
            ...
            <div className="mb-4">
                ...
                {plan.isPopular && (
                    <motion.span
                        className={cn(`inline-block px-3 py-1 text-white
                        font-bold rounded-md text-xs border-2 border-black
                        shadow-[2px_2px_0px_0px_rgba(0,0,0,0.9)]`, plan.accent)}
                        animate={{
                            y: [0, -3, 0],
                            scale: [1, 1.05, 1]
                        }}
                ...
                {plan.features.map((feature, i) => (
                    <motion.div
                        ...
                        <motion.span
                            ...
                            className={cn(`w-5 h-5 rounded-md  flex items-center justify-center
                            text-white font-bold text-xs border border-black
                            shadow-[1px_1px_0px_0px_rgba(0,0,0,0.9)]`, plan.accent)}>
                            ✓
                        </motion.span>
            ...
            {/* CTA Button */}
            <motion.button
                className={cn(`w-full py-2 rounded-lg  text-white font-black text-sm
                ...
                transition-all duration-200`, plan.accent)}
                whileHover={{
                    scale: 1.02,
...
export const PricingContainer = ({
    ...
    return (
        <div
            className={`min-h-screen bg-[#f0f0f0] p-4 sm:p-6 lg:p-8 relative overflow-hidden rounded-[12px] ${className}`}>
            <PricingHeader title={title} />
            <PricingToggle isYearly={isYearly} onToggle={() => setIsYearly(!isYearly)} />
            <BackgroundEffects />
            <div
                className="w-[100%] max-w-5xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 relative z-10">
